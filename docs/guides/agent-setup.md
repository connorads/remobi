# Configure remobi with AI agents

AI agents (Claude Code, Cursor, Copilot, etc.) can inspect your tmux config and generate a valid `remobi.config.ts` for you. This guide explains how to set that up, with a reference prompt you can paste directly.
...
## Install the remobi skill (recommended)
...
```bash
npx skills add connorads/remobi -a claude-code -g
```
...
> Inspect my tmux config (`tmux show-options -g prefix` and `tmux list-keys`), then generate a `remobi.config.ts` in the current directory tailored to my setup.
>
> Rules:
> - Use `import { defineConfig } from 'remobi'` and `export default defineConfig({...})`
...
>
> After writing the file, run `remobi build --dry-run` and fix any reported errors. Summarise what was configured and why.
...
## Tmux-to-remobi mapping reference
...
```
Dry run: build
- config: /path/to/remobi.config.ts
